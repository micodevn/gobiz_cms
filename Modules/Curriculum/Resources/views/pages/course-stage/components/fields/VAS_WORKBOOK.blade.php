@if(isset($oldStages))
    <div class="col-4 @if(!$checkTimeStage) disabled__select @endif">
        <select name="stage[{{$key_stage-1}}][content][{{$keyRd}}][{{$program_type_view}}][vas_id]"
                class="form-control" style="width: 100%;">
            <option value=""></option>
            @foreach($vas_workbook as $value)
                <option value="{{@$value['id']}}"
                        @if(@$item[$program_type_view]['vas_id'] == $value['id']) selected @endif>{{@$value['id']. '|' . $value['name']}}</option>
            @endforeach
        </select>
    </div>
...
@else
    <div class="col-4 @if(!$checkTimeStage) disabled__select @endif">
        <select name="stage[{{$key_stage-1}}][content][{{$keyRd}}][{{$program_type_view}}][vas_id]"
                class="form-control" style="width: 100%;">
            <option value=""></option>
            @foreach($vas_workbook as $val)
                <option value="{{$val['id']}}"
                        @if(@$data_field->vas_id == $val['id']) selected="selected" @endif>{{@$val['id']. '|' . $val['name']}}
                </option>
            @endforeach
        </select>
    </div>
