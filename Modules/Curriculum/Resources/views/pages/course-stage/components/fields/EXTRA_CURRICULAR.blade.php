@php
    $lessons = \App\Models\Lesson::query()->where('type', \App\Models\Lesson::TYPE_VIDEO)->select(['id', 'source', 'name'])->where('is_active', 1)->get();
@endphp
@if(isset($oldStages))
    @php
        $lesson_id = isset($item) ? @$item[$program_type_view]['lesson_id'] : null;
    @endphp
...
@else
    @php
        $lesson_id = isset($item) ? \Illuminate\Support\Arr::get($item, 'getProgramExtra.lesson_id') : null;
    @endphp
    <div class="col-4">
        <select required name="stage[{{$key_stage-1}}][content][{{$keyRd}}][{{$program_type_view}}][lesson_id][]"
                class="form-control lession__item__selected">
