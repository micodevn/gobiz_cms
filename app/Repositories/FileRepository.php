...
class FileRepository extends BaseRepository
{
    ...
    public function create(array $attributes, $create_physical_files = true)
    {
        ...
        $create_physical_files && $this->uploadFileAndIcon($attributes);
        $attributes['created_by'] = \Auth::id();

        !empty($attributes['file_path']) && $create_physical_files && $size = request()->file('file_path', null)->getSize();
        !empty($size) && $create_physical_files && $attributes['size'] = UpFileHelper::FileSizeConvert($size);
...
//        $timestamps = is_array($timestamps) ? $timestamps : json_decode($timestamps, true);
//        $subtitles = is_array($subtitles) ? $subtitles : json_decode($subtitles, true);

//        $this->syncWithLabel($created,$attributes);
//        $created->saveTimestamps($timestamps);
//        $created->saveSubtitles($subtitles);

//        foreach ($created->exercises as $exercise) {
//            $exercise->cache();
//        }
